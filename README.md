...
```bash
cv-website$ sam build --use-container
```
...
```bash
cv-website$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
cv-website$ sam logs -n HelloWorldFunction --stack-name "cv-website" --tail
```
...
```bash
sam delete --stack-name "cv-website"
```
